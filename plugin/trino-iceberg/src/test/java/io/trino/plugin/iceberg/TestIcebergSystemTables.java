    ...
    public void testFilesTable() throws IOException
    {
        try (TestTable table = new TestTable(getQueryRunner()::execute, "files_table", "(_bigint BIGINT, _date DATE) WITH (partitioning = ARRAY['_date'])")) {
            ...
            MaterializedResult expectedStatistics =
                    MaterializedResult.resultBuilder(getSession(), INTEGER, VARCHAR, BIGINT, BIGINT,
                                    new MapType(INTEGER, BIGINT, new TypeOperators()), new MapType(INTEGER, BIGINT, new TypeOperators()),
                                    new MapType(INTEGER, BIGINT, new TypeOperators()), new MapType(INTEGER, BIGINT, new TypeOperators()),
                                    VarbinaryType.VARBINARY, new ArrayType(BIGINT), new ArrayType(INTEGER),
                                    new MapType(INTEGER, VARCHAR, new TypeOperators()), new MapType(INTEGER, VARCHAR, new TypeOperators()))
